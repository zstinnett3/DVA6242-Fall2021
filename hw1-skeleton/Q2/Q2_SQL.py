...
class HW2_sql():
    ...
    # GTusername [0 points]
    def GTusername(self):
        gt_username = "zstinnett3"
        return gt_username

    # Part a.i Create Tables [2 points]
    def part_ai_1(self, connection):
        ############### EDIT SQL STATEMENT ###################################
        self.execute_query(connection, "DROP TABLE IF EXISTS movies;")
    ...
    def part_ai_2(self, connection):
        ############### EDIT SQL STATEMENT ###################################
        self.execute_query(connection, "DROP TABLE IF EXISTS movie_cast;")
    ...
    def part_aii_1(self, connection, path):
        ...
        connection.executemany("INSERT INTO movies (id, title, score) VALUES (?, ?,?);", movies)
        ######################################################################
    ...
    def part_aiii(self, connection):
        ...
        self.execute_query(connection, "DROP TABLE IF EXISTS cast_bio;")
    ...
    def part_c(self, connection):
        ############### EDIT SQL STATEMENT ###################################
        part_c_sql = '''SELECT printf('%.2f',AVG(case when ( lower(title) like '%war%' and score>50) 
    ...
    def part_d(self, connection):
        ############### EDIT SQL STATEMENT ###################################
        part_d_sql = part_c_sql = '''SELECT cast_name, 
    ...
    def part_e(self, connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = '''SELECT title as movie_title, 
                               printf('%.2f',score) as movie_score,
                               count(cast_id) as cast_count 
                        FROM movies m
                        INNER JOIN movie_cast c
                        on m.id=c.movie_id
                        group by title, score
                        order by score desc,count(cast_id) 
                        limit 5'''
        ######################################################################
    ...
    def part_g(self, connection):
        ############### EDIT SQL STATEMENT ###################################
        self.execute_query(connection, "DROP VIEW IF EXISTS good_collaboration;")

        part_g_sql = '''CREATE VIEW good_collaboration AS
                        select cast(a.cast_id as string) as cast_member_id1,
                               cast(b.cast_id as string) as cast_member_id2,
                               count(a.movie_id) as movie_count,
                               avg(a.score) as average_movie_score
                     from(
                        SELECT cast_id, 
                               movie_id, 
                               score
                        ...
                        FROM movies m
                        INNER JOIN movie_cast c
                        on m.id=c.movie_id)b
                        on a.movie_id=b.movie_id and a.cast_id > b.cast_id
                    group by cast(a.cast_id as string) ,
                             cast(b.cast_id as string)
                    having count(a.movie_id)>=3 and avg(a.score)>=40
                '''
        ######################################################################
    ...
    def part_gi(self, connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = ''' select * from (
                          select cast_id,trim(cast_name) as cast_name, printf('%.2f',avg(collaboration_score)) as collaboration_score 
                           FROM (
                                  SELECT  cast_member_id1 as cast_id,
                                   ...
                                   on mc.cast_id = gc.cast_member_id1
                                   group by cast_member_id1, cast_name
                               ...
                               FROM good_collaboration gc
                               inner join movie_cast mc
                               on mc.cast_id = gc.cast_member_id2
    ...
    def part_h(self, connection, path):
        ...
        self.execute_query(connection, "DROP TABLE IF EXISTS movie_overview;")
        ...
        connection.executemany("INSERT INTO movie_overview (id, overview) VALUES (?, ?);", movies)
        ######################################################################
    ...
    def part_hii(self, connection):
        ############### EDIT SQL STATEMENT ###################################
        part_hii_sql = '''SELECT count(*) from movie_overview where overview MATCH 'space NEAR/6 program'; '''
        ######################################################################
