...
class Graph:
    ...
    def add_node(self, id: str, name: str) -> None:
        ...
        """
        if (id, name.lower().strip().replace(',', '')) not in self.nodes:
            self.nodes.append((str(id), name.lower().strip().replace(',', '')))
        return
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        """
        if ((str(source), str(target)) not in self.edges) or ((str(target), str(source)) not in self.edges):
    ...
    def max_degree_nodes(self) -> dict:
        ...
        listIDs = [i[0] for i in self.nodes]

        listEdges_FirstSide = [i[0] for i in self.edges]
        listEdges_SecSide = [i[1] for i in self.edges]

        counts = Counter(listEdges_FirstSide) + Counter(listEdges_SecSide)

        max_degree = max(counts.values())

        ranks = {id: edges for (id, edges) in counts.items() if edges == max_degree}
        return ranks
    ...
    # Do not modify
    def write_edges_file(self,
                         path=r"C:\Users\zackarys\Dropbox\Graduate School\Data And Visual Analytics\hw1-skeleton\Q1\edges.csv") -> None:
        """
    ...
    def write_nodes_file(self,
                         path=r"C:\Users\zackarys\Dropbox\Graduate School\Data And Visual Analytics\hw1-skeleton\Q1\nodes.csv") -> None:
        """
...
class TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id: str, limit: int = None, exclude_ids: list = None) -> list:
        ...
        """
        try:
            exclude_ids = list(map(str, exclude_ids))
        except:
            pass
        url = "https://api.themoviedb.org/3/movie/{}/credits?api_key=49871dbee7aaa26179b87bd6b1791717&language=en-US".format(
            movie_id)
        request = Request(url)
        try:
            ...
            response.close()
            movie_cast = resp['cast'][:limit]
            movie_cast = [cast_member for cast_member in movie_cast[:limit] if str(cast_member['id']) not in exclude_ids]
    ...
    def get_movie_credits_for_person(self, person_id: str, vote_avg_threshold: float = None) -> list:
        ...
        """
        url = "https://api.themoviedb.org/3/person/{}/movie_credits?api_key=49871dbee7aaa26179b87bd6b1791717&language=en-US".format(
            person_id)
        ...
        try:
            ...
            response.close()
            credits = resp['cast']
            credits = [credit for credit in credits if credit['vote_average'] >= vote_avg_threshold]
        except:
            pass
        return credits
...
def return_argo_lite_snapshot()->str:
    ...
    """
    return 'https://poloclub.github.io/argo-graph-lite/#c92c341c-e889-4ee9-9a88-f90b9ee0c75e'

def get_costars(starting_id):
    starting_movies = tmdb_api_utils.get_movie_credits_for_person(person_id=str(starting_id),
                                                                  vote_avg_threshold=8.0)
    costars = []
    exclude_ids = [starting_id]

    for movie in starting_movies:
        movie_costars = tmdb_api_utils.get_movie_cast(movie['id'], limit=2, exclude_ids=exclude_ids)

        costars.append(movie_costars)
        try:
            exclude_ids.append(movie_costars[0]['id'])
            exclude_ids.append(movie_costars[1]['id'])
        except:
            pass

    for costar in list(itertools.chain(*costars)):
        if str(costar['id']) not in [x[0] for x in graph.nodes]:
            graph.add_node(costar['id'], costar['name'])
            costar_list.append(str(costar['id']))
        if (starting_id != str(costar['id'])):
            graph.add_edge(starting_id, costar['id'])
    graph.write_nodes_file()
    graph.write_edges_file()
    return costar_list
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='49871dbee7aaa26179b87bd6b1791717')

    # call functions or place code here to build graph (graph building code not graded)
    # Suggestion: code should contain steps outlined above in BUILD CO-ACTOR NETWORK

    costar_list = []
    starting_point = get_costars('2975')
    already_ran = []

    for costar in [i for i in starting_point]:
        get_costars(costar)
        already_ran.append(costar)

    for i in costar_list:
        print(len(graph.edges))
        if len(graph.edges) > 1600:
            break
